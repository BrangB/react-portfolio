...
function App() {
  ...
  return (
    <>
      <p className='flex md:hidden absolute top-6 right-6 p-3 z-40 rounded-full w-12 h-12 items-center justify-center duration-200 bg-[#E5EEFF] hover:bg-[#abb3c2]' onClick={() => setExpandMenu(true)}><i class="fa-solid fa-bars text-xl"></i></p>
      ...
      <div className={`${expandMenu ? 'scale-100' : 'scale-0'} fixed top-0 right-0 bg-transparent w-full h-full z-50 duration-200 flex flex-col items-center justify-center`}>
        <p className='top-5 right-5 flex lg:hidden fixed rounded-full duration-200 bg-transparent p-3 hover:bg-[#9797978f] w-12 h-12 items-center justify-center' onClick={() => setExpandMenu(false)}><i class="fa-solid fa-xmark text-2xl text-white"></i></p>
